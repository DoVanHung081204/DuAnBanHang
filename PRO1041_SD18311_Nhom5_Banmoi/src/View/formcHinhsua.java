...
public class formcHinhsua extends javax.swing.JFrame {
    ...
    public boolean checkDuLieu() {
        ...
        if (cboMaSp.getSelectedItem() == null || cboTP.getSelectedItem() == null) {
        ...
        try {
            ...
            int soLuongTaoCC = Integer.parseInt(txtCC.getText().trim());
            // Nếu dữ liệu không hợp lệ (ví dụ: số âm), hiển thị thông báo và trả về false
            if (soLuongTaoCC < 0) {
                JOptionPane.showMessageDialog(null, "Số lượng CC không được nhỏ hơn 0.", "Thông báo", JOptionPane.WARNING_MESSAGE);
                return false;
            }
        } catch (NumberFormatException e) {
            JOptionPane.showMessageDialog(null, "Số lượng CC không hợp lệ.", "Thông báo", JOptionPane.WARNING_MESSAGE);
            return false; // Nếu không thể chuyển đổi thành số, hiển thị thông báo và trả về false
        }

        try {
            // Kiểm tra kiểu dữ liệu của số lượng tạo
            int soLuongTaoCN = Integer.parseInt(txtCn.getText().trim());
            // Nếu dữ liệu không hợp lệ (ví dụ: số âm), hiển thị thông báo và trả về false
            if (soLuongTaoCN < 0) {
                JOptionPane.showMessageDialog(null, "Số lượng CN không được nhỏ hơn 0.", "Thông báo", JOptionPane.WARNING_MESSAGE);
                return false;
            }
        } catch (NumberFormatException e) {
            JOptionPane.showMessageDialog(null, "Số lượng CN không hợp lệ.", "Thông báo", JOptionPane.WARNING_MESSAGE);
            return false; // Nếu không thể chuyển đổi thành số, hiển thị thông báo và trả về false
        }
